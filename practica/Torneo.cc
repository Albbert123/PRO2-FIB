...
BinTree<int> Torneo::crear_emparejamiento(int profundidad, int raiz){
    int b = profundidad + 1 - raiz; 
    if(b<=ninscritos) { 
        BinTree<int> izquierdo = crear_emparejamiento(profundidad*2, raiz);
        BinTree<int> derecho = crear_emparejamiento(profundidad*2, b);
...
void Torneo::iniciar_torneo(const Cjt_Jugadores& cj){
    leer_inscritos(cj);
    emparejamiento = crear_emparejamiento(2,1);
