...
void Cjt_Jugadores::modificar_estadisticas(const string& nom, int puntos, int sets_g, int sets_p, int juegos_g,
    int juegos_p, int partidos_g, int partidos_p, int torneos_jugados) {
        ...
        double sg = it->second.consultar_sets_g();
        double sp = it->second.consultar_sets_p();
        double ratio = (sg*100)/(sg+sp);
        (*it).second.modificar_perc(ratio);
}
...
void Cjt_Jugadores::reordenar_ranking() {
    ...
    for(int i=0; i<njugadores; ++i) {
        ...
        //double div = it_ranking[i]->second.consultar_sets_g() / it_ranking[i]->second.consultar_sets_g() + it_ranking[i]->second.consultar_sets_p();
        (*it_ranking[i]).second.modificar_posicion(i + 1 - posicion);
    }
}

void Cjt_Jugadores::mejor_jugador() {
    float max = it_ranking[0]->second.consultar_perc();
    int pos = 0;

    for(int i = 0; i < njugadores; ++i) {
        if(max < it_ranking[i]->second.consultar_perc())
        {
            max = it_ranking[i]->second.consultar_perc();
            pos = i;
        }
        else if(max == it_ranking[i]->second.consultar_perc() and i < pos)
        {
            max = it_ranking[i]->second.consultar_perc();
            pos = i;
        }
    }
    cout << it_ranking[pos]->first << " " << max << endl;
}
